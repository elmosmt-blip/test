...
async function loadArticle() {
  ...
  const sourceTitle = primarySource.title || primarySource.url || 'SMTInsider Editorial';
  ...
  el.innerHTML = `
    ...
    <article class="site-preview" aria-label="Предпросмотр статьи в публичном формате">
      <div class="site-preview-hero">
        ...
        <aside class="preview-context"><div>REVIEW CONTEXT</div><dl><dt>CATEGORY</dt><dd>${escHtml(m.category || 'SMT Equipment')}</dd><dt>SOURCE</dt><dd>${escHtml(sourceTitle)}</dd><dt>AUTHOR</dt><dd>SMTInsider Editorial</dd><dt>STATUS</dt><dd>${factualPass ? 'Fact-checked draft' : 'Editorial draft — not publishable'}</dd><dt>LAST REVIEWED</dt><dd>${escHtml(publishedDate)}</dd></dl></aside>
      </div>
